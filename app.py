...
# optional dependency for thumbnails (requires poppler on Windows)
try:
...
def get_files_for(folder_name):
    folder = os.path.join(UPLOAD_FOLDER, folder_name)
    os.makedirs(folder, exist_ok=True)
    files = []
    for fname in sorted(os.listdir(folder), reverse=True):
        # skip thumbnail files from listing
        if fname.lower().endswith(('.png','.jpg','.jpeg')) and fname.startswith('thumb_'):
            continue
        url = f"/uploads/{folder_name}/{fname}"
        ext = os.path.splitext(fname)[1].lower()
        # check for generated thumbnail
        base = os.path.splitext(fname)[0]
        thumb_path = os.path.join(folder, f"thumb_{base}.png")
        thumb_url = f"/uploads/{folder_name}/thumb_{base}.png" if os.path.exists(thumb_path) else None
        files.append({"name": fname, "url": url, "ext": ext, "thumb": thumb_url})
    return files

@app.route("/articles")
def articles():
    files = get_files_for("articles")
    return render_template("articles.html", files=files)

@app.route("/venom")
def venom_page():
    files = get_files_for("venom")
    return render_template("venom.html", files=files)

@app.route("/talent")
def talent_page():
    files = get_files_for("talent")
...
def upload_file():
    ...
    target = request.form.get('targetLocation') or 'index'
    if not file_url:
        abort(400, "No URL provided")

    # convert common Google Docs/Drive view links to export/download links (PDF)
    if 'docs.google.com/document/d/' in file_url and 'export' not in file_url:
        file_url = file_url.rstrip('/') + '/export?format=pdf'
    elif 'drive.google.com/file/d/' in file_url and 'export' not in file_url:
        fid = file_url.split('/d/')[1].split('/')[0]
        file_url = f'https://drive.google.com/uc?export=download&id={fid}'
    elif 'drive.google.com/open' in file_url and 'id=' in file_url:
        fid = file_url.split('id=')[1].split('&')[0]
        file_url = f'https://drive.google.com/uc?export=download&id={fid}'

    headers = {'User-Agent': 'Mozilla/5.0'}
    r = requests.get(file_url, headers=headers, allow_redirects=True, timeout=30)
    if r.status_code != 200:
        abort(400, "Failed to fetch file; ensure it is shared publicly")

    folder_map = {
        'articles': os.path.join(UPLOAD_FOLDER, 'articles'),
        'venom': os.path.join(UPLOAD_FOLDER, 'venom'),
        'talent': os.path.join(UPLOAD_FOLDER, 'talent'),
        'index': os.path.join(UPLOAD_FOLDER, 'index'),
    }
    folder = folder_map.get(target, os.path.join(UPLOAD_FOLDER, 'index'))
    os.makedirs(folder, exist_ok=True)

    # determine filename and ensure .pdf when appropriate
    filename = os.path.basename(file_url.split('?')[0]) or 'document.pdf'
    ...
    # optional: generate thumbnail for PDFs if pdf2image/poppler available
    if filename.lower().endswith('.pdf') and PDF2IMAGE_AVAILABLE:
        try:
            images = convert_from_bytes(r.content, first_page=1, last_page=1, dpi=150)
            if images:
                thumb_name = f"thumb_{os.path.splitext(filename)[0]}.png"
                images[0].save(os.path.join(folder, thumb_name), "PNG")
        except Exception:
            pass
...
def delete_file():
    ...
    thumb_path = os.path.join(UPLOAD_FOLDER, folder, f"thumb_{os.path.splitext(filename)[0]}.png")
    try:
